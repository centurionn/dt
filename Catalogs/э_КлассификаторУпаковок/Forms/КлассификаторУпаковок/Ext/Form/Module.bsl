...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Дерево = Справочники.э_КлассификаторУпаковок.ПолучитьДанныеКлассификатора();
	...
	|	Упаковки.Код КАК Код
	|ИЗ
	|	Справочник.ВМТП_КлассификаторУпаковок КАК Упаковки
	|ГДЕ
	|	Упаковки.Код <> """"";
	Отбор = Неопределено;
...
Функция ОбработатьРезультатыПодбораНаСервере(ОбработкаПодбораВыполнена)
						...
						Для каждого СтрокаУровень3 Из СтрокаУровень2.Строки Цикл
							Если СтрокаУровень3.Выбран Тогда
								...
								Иначе
									...
									СправочникОбъект = Справочники.э_КлассификаторУпаковок.СоздатьЭлемент();
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось подобрать упаковку из классификатора.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(); // Для записи события в журнал регистрации.
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Подбор упаковок из классификатора.'", КодОсновногоЯзыка),
