...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Евросиб - АСуслов 20.03.2024 15:46:12 #
	//(
	Если НЕ ПараметрыСеанса.РаботаСВнешнимиРесурсамиЗаблокирована Тогда
		Элементы.ДобавитьПользователейТестовыхБаз.Видимость = Ложь;
	КонецЕсли;
	//)
	//Евросиб - АСуслов 20.03.2024 15:46:13 #
...
//Евросиб Коханенко Н.А. 16.06.2023 (( #70580 
&НаКлиенте
Процедура ДобавитьПользователейТестовыхБаз(Команда)
	ТекстСообщения = "Количество добавленных записей: " + ДобавитьПользователейТестовыхБазНаСервере();
	ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
КонецПроцедуры

&НаСервере
Функция ДобавитьПользователейТестовыхБазНаСервере()
	УстановитьПривилегированныйРежим(Истина);
	//получим список пользователей и групп, в которые необходимо добавить пользователей Тестовых Баз
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПользователиТестовыхБаз.Пользователь КАК ПользовательТБЗ,
		|	ПользователиТестовыхБаз.ГруппаДоступа КАК ГруппаДоступаТБЗ,
		|	ГруппыДоступа.Ссылка КАК ГруппаДоступаГлБазы
		|ИЗ
		|	РегистрСведений.ПользователиТестовыхБаз КАК ПользователиТестовыхБаз
		...
		|		ПО ПользователиТестовыхБаз.Пользователь = ГруппыДоступаПользователи.Пользователь
		|			И (ПользователиТестовыхБаз.ГруппаДоступа = ГруппыДоступа.Ссылка)
		|ГДЕ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ГруппаДоступаОбъект = ВыборкаДетальныеЗаписи.ГруппаДоступаТБЗ.ПолучитьОбъект();
		НоваяЗаписьГруппы = ГруппаДоступаОбъект.Пользователи.Добавить();
		НоваяЗаписьГруппы.Пользователь = ВыборкаДетальныеЗаписи.ПользовательТБЗ;
		ГруппаДоступаОбъект.Записать();
		КоличествоТранзакций = КоличествоТранзакций + 1;
	КонецЦикла;           
	УстановитьПривилегированныйРежим(Ложь);
	Возврат КоличествоТранзакций;
КонецФункции
//Евросиб Коханенко Н.А. 16.06.2023 )) #70580 
