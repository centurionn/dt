...
Процедура ПередЗаписью(Отказ)
	...
	//Евросиб - АСуслов 13.04.2023 13:09:46 #67444
	//(
	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Недействителен") <> Недействителен 
		И ЭтотОбъект.Недействителен Тогда
		ОтложенноеИзменениеСервер.ПоставитьСвязанныеОбъектыВОчередьИзменения(ЭтотОбъект);
	КонецЕсли;
	//)
	//Евросиб - АСуслов 13.04.2023 13:09:55 #67444  
	
	//Субботин 75158 17.10.2023 начало
	ЛогинАвторизацииОС = ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(ЭтотОбъект.ИдентификаторПользователяИБ).ПользовательОС;
	Если ЛогинАвторизацииОС <> ЛогинAD тогда
		 ЛогинAD = ЛогинАвторизацииОС;
	КонецЕсли;	
