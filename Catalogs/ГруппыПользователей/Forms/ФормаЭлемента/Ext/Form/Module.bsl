...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗадатьОтборПоГруппеОтветственных") и Не значениеЗаполнено(Объект.Ссылка) Тогда
		Объект.Родитель = Справочники.ГруппыПользователей.ГруппыОтветственныхЗаСопровождение
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтотОбъект.Модифицированность и ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		|ГДЕ
		|	ДоговорыКонтрагентовОтветственные.Ответственный = &Ответственный
		...
		|	И НЕ ДоговорыКонтрагентовОтветственные.Ссылка.ПометкаУдаления";
		
		Запрос.УстановитьПараметр("Ответственный", Объект.Ссылка);
		
		РезультатЗапроса = Запрос.Выполнить();
		
		Если не РезультатЗапроса.Пустой() Тогда
			ОбщегоНазначения.СообщитьПользователю("Данная группа уже используется в договорах. Для изменения состава необходимо создать новую группу.",,,,Отказ);
		КонецЕсли;
	КонецЕсли; 	
