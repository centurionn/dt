...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//принудительно Блокируем команды для заявок на оплату
	Если ТипЗнч(Объект.ВладелецФайла) = Тип("ДокументСсылка.ЗаявкаНаОплату") Тогда
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоСканКопия Тогда
		Если ТипЗнч(Объект.ВладелецФайла) = Тип("ДокументСсылка.ПСЦПоставщика") Тогда
			
			Запрос = Новый Запрос;
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ПроцессСогласования.СогласованиеЗавершено КАК СогласованиеЗавершено
			|ИЗ
			|	Справочник.ПроцессСогласования КАК ПроцессСогласования
			|ГДЕ
			|	ПроцессСогласования.ДокументСогласования = &ДокументСогласования
			|	И ПроцессСогласования.СогласованиеЗавершено";
			
			Запрос.УстановитьПараметр("ДокументСогласования", Объект.ВладелецФайла);
			
			Если НЕ Запрос.Выполнить().Пустой() Тогда
				ЗадачиНаВыполнение = Новый Массив;
				ЗадачиНаВыполнение.Добавить(СПравочники.Состояния.ПроверкаСогласованияОтправкаПодкреплениеПодписанногоПСЦ);
				МаршрутизацияСервер.ОтметитьВыполнениеЗадачПоСсылке(Объект.ВладелецФайла,ЗадачиНаВыполнение);
			КонецЕсли; 
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	ЭтоСканКопия(Истина); //Евросиб Шевченко А.В. US 110215 TA 122847 10.11.2025
...
&НаСервере
Процедура ЭтоСканКопия(Запись = Ложь) //Евросиб Шевченко А.В. US 110215 TA 122847 10.11.2025
	Если Объект.ЭтоСканКопия Тогда
		Если ТипЗнч(Объект.ВладелецФайла) = Тип("ДокументСсылка.ПСЦПоставщика") Тогда
			...
			Если Запрос.Выполнить().Пустой() Тогда
				Если НЕ Запись Тогда
					ОбщегоНазначения.СообщитьПользователю("Невозможно подкрепить подписанную скан-копию пока не будет завершено согласование и не будет сформирован Лист согласования");
					Объект.ЭтоСканКопия = НЕ Объект.ЭтоСканКопия;
				КонецЕсли;
			Иначе
				Если Запись Тогда
					ЗадачиНаВыполнение = Новый Массив;
...
&НаКлиенте
Процедура ЭтоСканКопияПриИзменении(Элемент)
	ЭтоСканКопия();
КонецПроцедуры
