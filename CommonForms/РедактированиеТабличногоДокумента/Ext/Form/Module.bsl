...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Параметры.ТабличныйДокумент) = Тип("ТабличныйДокумент") Тогда
		...
		Если Параметры.Свойство("НеПредлагатьСохранениеВФайл") Тогда
			НеПредлагатьСохранениеВФайл = Параметры.НеПредлагатьСохранениеВФайл;
		КонецЕсли;
		//Евросиб Коханенко Н.А. 30.10.2024 )) #92915 
	Иначе
...
Процедура ЗагрузитьТабличныйДокументИзМетаданных()
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		ТабличныйДокумент = МодульУправлениеПечатью.МакетПечатнойФормы(Параметры.ИмяОбъектаМетаданныхМакета, Контрагент, Договор);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьТабличныйДокумент(ОбработчикЗавершения = Неопределено)
	...
	Если НеПредлагатьСохранениеВФайл = Ложь 
		И (ЭтоНовый()
		ИЛИ РедактированиеЗапрещено) Тогда
	//Евросиб Коханенко Н.А. 30.10.2024 )) #92915 
...
//Евросиб Коханенко Н.А. 30.10.2024 (( #92915 
&НаСервере
Функция НеПредлагатьСохранениеВФайл()
	Возврат Параметры.Свойство("НеПредлагатьСохранениеВФайл") И Параметры.НеПредлагатьСохранениеВФайл;
