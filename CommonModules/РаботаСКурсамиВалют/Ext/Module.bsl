...
//
//Функция ПересчитатьВВалюту(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс = неопределено, НеОкруглятьРезультат = Ложь) Экспорт
//ДИТ, Беляев СЮ, BUG 76518 / TA 76823, 03-11-2023, добавлен параметр "ОкруглениеКурса"
Функция ПересчитатьВВалюту(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс = неопределено, НеОкруглятьРезультат = Ложь, ОкруглениеКурса = Неопределено) Экспорт
	
	//Если Произвольныйкурс = неопределено Тогда
	//	Возврат РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма, ПолучитьКурсВалюты(ИсходнаяВалюта, Дата), ПолучитьКурсВалюты(НоваяВалюта, Дата), НеОкруглятьРезультат);
	//Иначе
	//	Возврат ПересчитатьВВалютуПоПроизвольномуКурсу(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс, НеОкруглятьРезультат);
	//КонецЕсли;
	
	//ДИТ, Беляев СЮ, BUG 76518 / TA 76823, 03-11-2023, начало
	Если Произвольныйкурс = неопределено Тогда
		Возврат РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма, ПолучитьКурсВалюты(ИсходнаяВалюта, Дата), ПолучитьКурсВалюты(НоваяВалюта, Дата), НеОкруглятьРезультат, ОкруглениеКурса);
	Иначе
		Возврат ПересчитатьВВалютуПоПроизвольномуКурсу(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс, НеОкруглятьРезультат, ОкруглениеКурса);
	КонецЕсли;
...
//  Число - пересчитанная сумма.
//
//Функция ПересчитатьВВалютуПоПроизвольномуКурсу(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс, НеОкруглятьРезультат = Ложь) Экспорт
//ДИТ, Беляев СЮ, BUG 76518 / TA 76823, 03-11-2023, добавлен параметр "ОкруглениеКурса"
Функция ПересчитатьВВалютуПоПроизвольномуКурсу(Сумма, ИсходнаяВалюта, НоваяВалюта, Дата, ПроизвольныйКурс, НеОкруглятьРезультат = Ложь, ОкруглениеКурса = Неопределено) Экспорт
	
	РЕЗ1 = ПолучитьКурсВалюты(ИсходнаяВалюта, Дата);
	РЕЗ2 = ПолучитьКурсВалюты(НоваяВалюта, Дата);
	//Евросиб - АСуслов 13.11.2020 15:39:51 #
	//(
	Если ИсходнаяВалюта = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьЗначениеКонстанты("ВалютаРегламентированногоУчета") Тогда
		РЕЗ2.Курс = ПроизвольныйКурс;
	Иначе
		РЕЗ1.Курс = ПроизвольныйКурс;
	КонецЕсли; 
	//)
	//Евросиб - АСуслов 13.11.2020 15:39:51 #
	
	//Возврат РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма,РЕЗ1,РЕЗ2, НеОкруглятьРезультат);
	//ДИТ, Беляев СЮ, BUG 76518 / TA 76823, 03-11-2023, начало
	Возврат РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Сумма,РЕЗ1,РЕЗ2, НеОкруглятьРезультат, ОкруглениеКурса);
	//ДИТ, Беляев СЮ, BUG 76518 / TA 76823, 03-11-2023, конец	
КонецФункции
