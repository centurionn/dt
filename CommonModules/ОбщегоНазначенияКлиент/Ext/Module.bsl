...
///////// DATACore
Процедура ОткрытьНакладнуюПоНомеру(НомерНакладной = неопределено, П2 = неопределено) Экспорт
	
	Если не ЗначениеЗаполнено(НомерНакладной) Тогда Возврат; КонецЕсли;
	
	Накладная = ОбщегоНазначенияВызовСервера.НайтиНакладнуюПоНомеру(НомерНакладной);
	
	Если ЗначениеЗаполнено(Накладная) тогда ПоказатьЗначение(,Накладная); Иначе Сообщить("Не найдена накладная с номером " + НомерНакладной); КонецЕсли;
	
КонецПроцедуры

Процедура ОбработкаОтветаНаОбновлениеСтопЛиста(П1 = неопределено, П2 = неопределено) Экспорт
	
	Если П1 <> КодВозвратаДиалога.Да Тогда Возврат; КонецЕсли;
	
	КонтрагентыДоговоры_сервер.СтопЛистОбновление();
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	Возврат ОбщегоНазначенияВызовСервера.ЗначенияРеквизитовОбъекта(Ссылка, Реквизиты, ВыбратьРазрешенные);		
КонецФункции

Функция ЗначенияРеквизитовОбъектов(Ссылки, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	Возврат ОбщегоНазначенияВызовСервера.ЗначенияРеквизитовОбъектов(Ссылки, Реквизиты, ВыбратьРазрешенные);		
КонецФункции
