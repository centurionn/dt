...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	// ++ ДИТ Сгонник, 2021-11-03 otrs TicketID=242182
	//Запрос = Новый Запрос(ТекстЗапросаДляИзвлеченияТекста());
	ТекстЗапросаДляИзвлеченияТекста = ТекстЗапросаДляИзвлеченияТекста();
	Если ТекстЗапросаДляИзвлеченияТекста = "" Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Файлы.Извлечение текста'",
			     ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,
