...
Функция ДобавитьФайл(ПараметрыФайла,
	...
	/// Шейкина
	Попытка
		Описание = ПараметрыФайла.Описание;
	Исключение
	КонецПопытки;
	// Шейкина
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	//Евросиб Каблов 02.09.2022 ( Task 0279728
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		ЗавершатьРедактированиеФайлов = Истина;
	Иначе
		ЗавершатьРедактированиеФайлов = Ложь;
	КонецЕсли;
	//Евросиб Каблов 02.09.2022 )
	...
	Для каждого ОписаниеИмениСправочника Из ИменаСправочников Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				//Евросиб Каблов 02.09.2022 ( Task 0279728
				Если ЗавершатьРедактированиеФайлов Тогда
					РаботаСФайламиСлужебныйВызовСервера.ОсвободитьФайл(Выборка);
				Иначе
				//Евросиб Каблов 02.09.2022 )
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
