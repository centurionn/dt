...
//
Функция МакетПечатнойФормы(ПутьКМакету, Контрагент = неопределено, Договор = неопределено) Экспорт
	...
	ТекстЗапроса =  "ВЫБРАТЬ ПЕРВЫЕ 1
	                ...
	                |	ПользовательскиеМакетыПечати.Контрагент КАК Контрагент,
	                |	ПользовательскиеМакетыПечати.Договор КАК Договор
	                |ИЗ
	                ...
	                |	И (ПользовательскиеМакетыПечати.Контрагент = &Контрагент
	                |			ИЛИ ПользовательскиеМакетыПечати.Контрагент = &ПустойКонтрагент)
	                |	И (ПользовательскиеМакетыПечати.Договор = &Договор
	                |			ИЛИ ПользовательскиеМакетыПечати.Договор = &ПустойДоговор)
	                |
	                |УПОРЯДОЧИТЬ ПО
	                |	ПользовательскиеМакетыПечати.Контрагент УБЫВ,
	                |	ПользовательскиеМакетыПечати.Договор УБЫВ";
	...
	Запрос.Параметры.Вставить("ПустойДоговор", Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Иначе
		Если МассивОбъектов.Количество() > 0 И МассивОбъектов[0] <> неопределено Тогда
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		//добавим ссылку в коллекцию
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(МассивОбъектов)) Тогда
			Для каждого СтрокаКоллекции Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
				СтрокаКоллекции.СсылкаДок = МассивОбъектов; 
			КонецЦикла; 
		Иначе
			Если МассивОбъектов.Количество() > 0 И МассивОбъектов[0] <> неопределено Тогда
				Для каждого СтрокаКоллекции Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
					СтрокаКоллекции.СсылкаДок = МассивОбъектов[0];  
				КонецЦикла; 
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого СтрокаКоллекции Из ВременнаяКоллекцияДляОднойПечатнойФормы Цикл
			Если ПараметрыПечати.Свойство("СсылкаНаДокументПервоначальная") Тогда
				СтрокаКоллекции.СсылкаДокПервоначальная = ПараметрыПечати.СсылкаНаДокументПервоначальная;
			КонецЕсли;
		КонецЦикла;
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		//Евросиб 10.12.2020 ( Ticket#0212015
		Если ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
			СписокРеквизитов = "ДатаДокумента,НомерДляПечати,Номер,Дата";
			Шаблон = НСтр("ru = '[НазваниеПечатнойФормы] № [НомерДляПечати] от [ДатаДокумента]'");
		КонецЕсли;
		//Евросиб 10.12.2020 )
		
		//СиДата 15.12.2020 ( Ticket#0216352
		Если ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.ПСЦ") Тогда
			СписокРеквизитов = "ДатаПСЦ,НомерПСЦ";
			Шаблон = НСтр("ru = 'Протокол № [НомерПСЦ] от [ДатаПСЦ]'");
		КонецЕсли;
		//СиДата 15.12.2020 )
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрефиксацияОбъектов") И ДокументСодержитНомер Тогда
			...
			Если ПараметрыДляВставки.Свойство("Номер") Тогда
		...
		Если ТипЗнч(ОбъектПечати) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		    ПараметрыДляВставки.ДатаДокумента = Формат(ПараметрыДляВставки.ДатаДокумента, "ДЛФ=D");
			ПараметрыДляВставки.НомерДляПечати = СтрЗаменить(ПараметрыДляВставки.НомерДляПечати, "/", " ");
		Иначе
			
			Если ПараметрыДляВставки.Свойство("Дата") Тогда
				ПараметрыДляВставки.Дата = Формат(ПараметрыДляВставки.Дата, "ДЛФ=D"); 
			КонецЕсли; 
...
Функция ИмяФайлаОфисногоДокумента(Знач ИмяФайла, Знач ПереводитьВТранслит = Ложь) Экспорт
	...
	Если Не СтрЗаканчиваетсяНа(ИмяФайла, ".odt") И Не СтрЗаканчиваетсяНа(ИмяФайла, ".docx") И Не СтрЗаканчиваетсяНа(ИмяФайла, ".pdf") Тогда
