...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Если ПараметрыКлиента.ДоступноИспользованиеРазделенныхДанных Тогда
		...
		Попытка
			ПредставлениеКонфигурации = ПараметрыКлиента.ИмяТекущегоПользователя;
		Исключение
		КонецПопытки;
	...
	Если ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована")
		...
		//Евросиб - АСуслов 10.01.2021 20:08:22 #
		//(
		СтрокаСоединения = СтрокаСоединенияИнформационнойБазы();
		НачальныйИндексИмениБазы = СтрНайти(СтрокаСоединения,"Ref=") + 5;
		КонечныйИндексИмениБазы = СтрНайти(СтрокаСоединения, """",,НачальныйИндексИмениБазы);
		ИмяБазы = Сред(СтрокаСоединения, НачальныйИндексИмениБазы, КонечныйИндексИмениБазы - НачальныйИндексИмениБазы);
		ЗаголовокПриложения = НСтр("ru = '[КОПИЯ %1]'") + " " + ЗаголовокПриложения;
		ЗаголовокПриложения = СтрШаблон(ЗаголовокПриложения, ИмяБазы);
		//)
		//Евросиб - АСуслов 10.01.2021 20:08:22 #
	КонецЕсли;
	
	//Евросиб - АСуслов 28.04.2023 13:12:55 #
	//(
	ЗаголовокПриложения = СтрШаблон("%1 / Версия: %2 /", ЗаголовокПриложения, ?(ПриЗапуске, ПараметрыКлиента.ВерсияДанныхОсновнойКонфигурации, ПараметрыКлиента.ВерсияКонфигурации));
	//)
	//Евросиб - АСуслов 28.04.2023 13:12:56 #
