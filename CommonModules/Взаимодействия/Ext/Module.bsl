...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено, ПараметрыПисьма = Неопределено, ПротоколПочты = "") Экспорт
	...
	ПараметрыПисьма.Вставить("ПисьмоСсылка", Объект.Ссылка);
	...
	//ЗафиксироватьОтправкуПисьма(Объект);	
...
// ДИТ US 72708 ТА 72713 Сержантова А.С. Начало: 11.09.2023 Ticket#0366965 06.10.2024 - перенесено после отправки письма
//Процедура ЗафиксироватьОтправкуПисьма(Объект)  
//	
//	АдресаОтправки = СтрСоединить(Объект.ПолучателиПисьма.Выгрузить().ВыгрузитьКолонку("Адрес"), "; ");
//	
//	Для каждого Стр Из Объект.ВладельцыПФ Цикл   
...
//			ЗаписьРС.ОтправленныйДокумент 	= Стр.Документ;
//			ЗаписьРС.ДатаОтправки 			= ТекущаяДата();
//			//Письмо было удалено после отправки
//			ЗаписьРС.ИсходящееПисьмо 		= Объект.Ссылка;
//			ЗаписьРС.АдресаОтправки 		= АдресаОтправки;	
//			ЗаписьРС.Отправитель 			= Объект.Ответственный;
//			ЗаписьРС.Записать();
//		КонецЕсли;
//	КонецЦикла;
//	
...
//Евросиб - АСуслов 07.08.2020 12:52:39 #
//(
	                                   Объект.Текст, СтрЗаменить(Объект.ТекстHTML,"%ПереченьДокументов%",Объект.ТекстТаблицаВложенныхФайловHTML)));
