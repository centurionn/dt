...
Процедура ВыполнитьРегламентныеДействия(Обработка, ПараметрыДействия = Неопределено) Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.Диадок_РегламентныеДействия);
	//Евросиб Каблов 08.10.2020 )
...
//Евросиб Каблов 22.06.2021 Ticket#0225310
Функция ПолучитьПечатнуюФормуДокументаПоСсылке(ДокументДД) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ОбработкаОбъект = СоздатьОбработкуКонтурЭДО("КонтурЭДО");
	ПечатнаяФорма = ОбработкаОбъект.ПолучитьПечатнуюФормуДокументаПоСсылке(ДокументДД);
