...
Функция СформироватьСписокПолучателейРассылки(Знач Параметры) Экспорт
	...
	ТипПолучателей = Параметры.Ссылка.ТипПолучателейРассылки.КлючОбъектаМетаданных.Получить();
	
	Если Метаданные.НайтиПоТипу(ТипПолучателей) = Метаданные.РегистрыСведений.НастройкиРассылкиПДЗ Тогда
		РезультатВыполнения.Получатели = РегистрыСведений.НастройкиРассылкиПДЗ.ПолучателиДоговораПоДаннымРегистра();
	КонецЕсли;
