...
Функция ИнформацияОбУстаревшихВерсиях()
	...
	Запрос = ЗапросПоУстаревшимВерсиямОптимизация(ТекстЗапроса);
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция ЗапросПоУстаревшимВерсиямОптимизация(ТекстЗапроса)
	...
	Для Индекс = 0 По ГраницыУдаленияОбъектов.Количество() - 1 Цикл
		...
		Если Индекс = 0 Тогда
			Запрос.Текст = ТекстЗапроса + "И" + ДополнительныеУсловия;
		Иначе
			Запрос.Текст = Запрос.Текст +  " 
			|ОБЪЕДИНИТЬ ВСЕ 
			|" + ТекстЗапроса + " И " + ДополнительныеУсловия;
		КонецЕсли;
	КонецЦикла;
	...
	Если ГраницыУдаленияОбъектов.Количество() > 1 Тогда
		Запрос.Текст = "ВЫБРАТЬ
						|	СУММА(ВЗ.РазмерДанных) КАК РазмерДанных,
						|	СУММА(ВЗ.КоличествоВерсий) КАК КоличествоВерсий
						|ИЗ (" + Запрос.Текст + ") КАК ВЗ";
	КонецЕсли;
