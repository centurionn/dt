...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	ПолучателиПоДаннымРегистра = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ДополнительныеНастройки <> Неопределено И ДополнительныеНастройки.Свойство("Получатели") Тогда
			...
			Если Метаданные.НайтиПоТипу(ТипПолучателей) = Метаданные.РегистрыСведений.НастройкиРассылкиПДЗ Тогда
				ПолучателиПоДаннымРегистра = Истина;
			КонецЕсли;
			// ДИТ 56017 Давудов М.А. Конец: 13.07.202
		...
		Иначе
			...
			Если Метаданные.НайтиПоТипу(ТипПолучателей) = Метаданные.РегистрыСведений.НастройкиРассылкиПДЗ Тогда
				ПолучателиПоДаннымРегистра = Истина;
				Получатели = РегистрыСведений.НастройкиРассылкиПДЗ.ПолучателиДоговораПоДаннымРегистра();
			КонецЕсли;
			// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	ПараметрыДоставки.Вставить("ПолучателиПоДаннымРегистра", ПолучателиПоДаннымРегистра);
	// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
...
Функция ВыполнитьРассылку(Отчеты, ПараметрыДоставки, НаименованиеРассылки = "", ПараметрыЖурнала = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтчет Из Отчеты Цикл
		...
		Попытка
			...
			//ок)
			Если ПараметрыОтчета.Персонализирован
			 ИЛИ ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
			// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	ОбщиеВложенияПолучателиПоДаннымРегистра = Новый Массив;
	МассивРассылкиПоДоговорам = Новый Массив;
	// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
		Если РассылкаВыполнена
		   И ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
			Для Каждого ЭлементВложения Из ЛичныеВложения Цикл
				ОбщиеВложенияПолучателиПоДаннымРегистра.Добавить(ЭлементВложения);
			КонецЦикла;
			МассивРассылкиПоДоговорам.Добавить(СтрокаПолучатель.Ключ);
		КонецЕсли;
		// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	Если ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
		Для Каждого ЭлементВложения Из ОбщиеВложенияПолучателиПоДаннымРегистра Цикл
			ОбщиеВложения.Добавить(ЭлементВложения);
		КонецЦикла;
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
	КонецЕсли;
	// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	Если РассылкаВыполнена
	   И ПараметрыДоставки.ПолучателиПоДаннымРегистра
	   И МассивРассылкиПоДоговорам.Количество() > 0 Тогда
		
		Участники = Константы.ГруппаПолучателейПоРассылкеПДЗ.Получить();
		...
		Если ТипЗнч(Участники) = Тип("СправочникСсылка.Пользователи") Тогда
			НайденныеСтроки = Участники.КонтактнаяИнформация.НайтиСтроки(СтруктураОтбора);
			Если НайденныеСтроки.Количество() > 0
			   И ЗначениеЗаполнено(НайденныеСтроки[0].Представление) Тогда
				ПолучателиПисьма.Добавить(НайденныеСтроки[0].Представление);
			КонецЕсли;
		Иначе
			Участники = Пользователи_sd_сервер.ПолучитьПользователейГруппы(Участники);
			Для Каждого Участник из Участники Цикл
				НайденныеСтроки = Участник.КонтактнаяИнформация.НайтиСтроки(СтруктураОтбора);
				Если НайденныеСтроки.Количество() > 0
				   И ЗначениеЗаполнено(НайденныеСтроки[0].Представление)
				   И ПолучателиПисьма.Найти(НайденныеСтроки[0].Представление) = Неопределено Тогда
					ПолучателиПисьма.Добавить(НайденныеСтроки[0].Представление);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если ПолучателиПисьма.Количество() > 0 Тогда
			...
			ПараметрыПисьма.Вставить("Тема", 
				НСтр("ru = 'Список отправленных информационных писем по ПДЗ'"));
				
				ТелоПисьма = "";
			Для Каждого Договор Из МассивРассылкиПоДоговорам Цикл
				ТелоПисьма = ТелоПисьма + ?(ЗначениеЗаполнено(ТелоПисьма), Символы.ПС, "") + 
					"Контрагент:  """ + Строка(Договор.Владелец) + """ Договор: """ + Строка(Договор) + """.";
			КонецЦикла;
			
			ПараметрыПисьма.Вставить("Тело", ТелоПисьма);
			ПараметрыПисьма.Вставить("Кому", СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(ПолучателиПисьма, ";"));
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	Если Метаданные.НайтиПоТипу(ТипПолучателей) = Метаданные.РегистрыСведений.НастройкиРассылкиПДЗ Тогда
		//СписокПолучателей.Вставить("РегистрСведений.НастройкиРассылкиПДЗ");
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если ЗначениеЗаполнено(Получатель)
	   И ТипЗнч(Получатель) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ПользовательскиеНастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки;
		
		Для Каждого ЭлементОтбора Из ПараметрыОтчета.НастройкиКД.Отбор.Элементы Цикл
			Если ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Договор") Тогда
				Настройка = 
					ПользовательскиеНастройкиКД.Элементы.Найти(ЭлементОтбора.ИдентификаторПользовательскойНастройки);
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	Если ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
		ПредставлениеОтчетаИсходный = ПредставлениеОтчета;
		ПредставлениеОтчета = Формат(ТекущаяДатаСеанса(), "ДЛФ=D") + " " +Строка(ПолучательСсылка.Владелец) + " " + 
			Строка(ПолучательСсылка) + " ";
		СтрокаОтчет.Ключ = ПредставлениеОтчета;
	КонецЕсли;
	// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
	...
	// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
	Если ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
		СтрокаОтчет.Настройки.Вставить("ПредставлениеВПисьме", ПредставлениеОтчетаИсходный);
	Иначе
		//ок(
...
Процедура ОтправитьОтчетыПолучателю(Вложения, ПараметрыДоставки, СтрокаПолучатель = Неопределено)
	...
	Иначе
		...
		// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
		Если ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
			ПараметрыПисьма.Тема = СтрЗаменить(ШаблонТемы, "[Контрагент]", Строка(Получатель.Владелец));
			ПараметрыПисьма.Тема = СтрЗаменить(ПараметрыПисьма.Тема, "[Договор]", Строка(Получатель));
			РегистрыСведений.НастройкиРассылкиПДЗ.ЗаполнитьПараметрыПисьмаПоДоговору(ПараметрыПисьма, Получатель);
		Иначе
		//ок(
		// Получатель
		...
		// ДИТ 56017 Давудов М.А. Начало: 13.07.2023
		Если ПараметрыПисьма.Свойство(КлючАдресаДоставки) Тогда
		//ок(
		// Отправляем письмо
		ОтправитьПочтовоеСообщение(ПараметрыДоставки, ПараметрыПисьма);
		//ок)
			Если ПараметрыДоставки.ПолучателиПоДаннымРегистра Тогда
				Если ПараметрыПисьма.Свойство(КлючАдресаДоставки) Тогда
					ПараметрыПисьма.Удалить(КлючАдресаДоставки);
				КонецЕсли;
				Если ПараметрыПисьма.Свойство("Копии") Тогда
					ПараметрыПисьма.Удалить("Копии");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		// ДИТ 56017 Давудов М.А. Конец: 13.07.2023
