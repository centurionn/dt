...
Процедура ВыполнитьКомандуПечати(ИмяМенеджераПечати, ИменаМакетов, МассивОбъектов, ВладелецФормы, ПараметрыПечати = Неопределено) Экспорт
	...
	Попытка    // у формы может не быть объекта
		Если ПараметрыПечати <> Неопределено и ПараметрыПечати.Свойство("ДополнительныеПараметры") 
			и ЗначениеЗаполнено(ВладелецФормы.Объект.Ссылка) Тогда
			ПараметрыПечати.ДополнительныеПараметры.Вставить("СсылкаНаДокументПервоначальная", ВладелецФормы.Объект.Ссылка);
		КонецЕсли;   
...
//  ВладелецФормы - УправляемаяФорма - форма, из которой выполняется печать.
//
Процедура ПечатьДокументов(КоллекцияПечатныхФорм, Знач ОбъектыПечати = Неопределено, ВладелецФормы = Неопределено, Знач ПараметрКоманды = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	//ДИТ Пак И.Т. Task34787 (Ticket#0206504) 10.06.2021 Начало
	// ----- старый текст ------
	//
	...
	Если ДопПараметры <> Неопределено Тогда
		Если ТипЗнч(ДопПараметры) = Тип("Структура") Тогда
			Для каждого ЭлСтруктуры Из ДопПараметры Цикл
				Если ЭлСтруктуры.Ключ = "ОписаниеОповещенияОЗакрытии" Тогда
					ОписаниеОповещенияОЗакрытии = ЭлСтруктуры.Значение;
				Иначе
					ПараметрыОткрытия.Вставить(ЭлСтруктуры.Ключ, ЭлСтруктуры.Значение);
				КонецЕсли;
			КонецЦикла;
		Иначе
	...
	ОткрытьФорму("ОбщаяФорма.ПечатьДокументов", ПараметрыОткрытия, ВладелецФормы, КлючУникальности,,, ОписаниеОповещенияОЗакрытии);
