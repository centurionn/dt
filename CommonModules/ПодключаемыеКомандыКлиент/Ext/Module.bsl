...
Процедура ВыполнитьКоманду(Форма, Команда, Источник) Экспорт
	...
	Если ОписаниеКоманды.ДополнительныеПараметры.Свойство("ДополнительныеПараметры") тогда
		Если ОписаниеКоманды.ДополнительныеПараметры.ДополнительныеПараметры.Свойство("НеобходимаПроверкаРолиДляПечати") Тогда;
			Контекст.Вставить("НеобходимаПроверкаРоли", Истина);
		КонецЕсли;
		// Евросиб US 89967 TA 91565 Рыбьяков К.А. 25.10.2024 ((
		Если ОписаниеКоманды.ДополнительныеПараметры.ДополнительныеПараметры.Свойство("ПроверкаЗапретаПечатиДляСтавкиНДС20") Тогда
			Контекст.Вставить("ПроверкаЗапретаПечатиДляСтавкиНДС20", Истина);
		КонецЕсли;
		// Евросиб Рыбьяков К.А. 25.10.2024 ))
		// Евросиб US 90342 Рыбьяков К.А. 10.03.2026 ((
		Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.РеестрСпециальныхСтавок") 
			И ОписаниеКоманды.ДополнительныеПараметры.Свойство("ДополнительныеПараметры") Тогда
			Контекст.ОписаниеКоманды.ДополнительныеПараметры.ДополнительныеПараметры.Вставить("ВыбранныеКонтрагенты", Форма.ВыбранныеКонтрагенты);
		КонецЕсли;
...
Процедура ПродолжитьВыполнениеКоманды(Контекст)
	...
	Если Контекст.Свойство("НеобходимаПроверкаРоли") Тогда
		Для Каждого Док из Контекст.МассивСсылок Цикл
			Если (ТипЗнч(Док) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				и ОбщегоНазначенияКлиент.ЗначениеРеквизитаОбъекта(Док,"Штраф"))
				//условие 2
				или  (ТипЗнч(Док) = Тип("ДокументСсылка.СчетФактураВыданный") 
				  и (ОбщегоНазначенияКлиент.ЗначениеРеквизитаОбъекта(Док,"Штрафы")
				  или НЕ КонтрольДоступностиДокументов_сервер.ПроверитьВозможностьРаботыСДокументомВызовСервера(Док))) Тогда
				Если НЕ КонтрольДоступностиДокументов_сервер.РольДоступнаВызовСервера("dc_РазрешитьОтправкуШтрафныхРТУ") Тогда				
					ВызватьИсключение("Недостаточно прав для выполнения операции. Выбранный документ имеет признак Штраф");
				КонецЕсли;
			КонецЕсли;
			//Евросиб СубботинВ #119324 07.10.2025 Начало
			Если ТипЗнч(Док) = Тип("ДокументСсылка.РеализацияТоваровУслуг") и НЕ КонтрольДоступностиДокументов_сервер.РольДоступнаВызовСервера("dc_РазрешитьОтправкуШтрафныхРТУ") Тогда
				КонтрольДоступностиДокументов_сервер.ПроверитьВозможностьПечатиРтуПоСпискуНоменклатур(Док);
            КонецЕсли;
	...
	// Евросиб US 89967 TA 91565 Рыбьяков К.А. 25.10.2024 ((
	Если Контекст.Свойство("ПроверкаЗапретаПечатиДляСтавкиНДС20") И Контекст.Свойство("МассивСсылок") Тогда
		НеДоступныДляПечати = ПечатьБухгалтерскихДокументов.ДоступныеДляПечатиИОтправкиПоЭдо(Контекст.МассивСсылок);
