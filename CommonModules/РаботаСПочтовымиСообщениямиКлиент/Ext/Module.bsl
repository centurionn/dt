...
Процедура ОбработатьВопросПользователяПоСрочномуНапоминанию(П1 = неопределено, П2 = неопределено) Экспорт
	
	Если П1 = КодВозвратаДиалога.Да Тогда
		РаботаСМаршрутизациейДокументов.НапомнитьПользователямОЗадачах(П2);
	КонецЕсли;
