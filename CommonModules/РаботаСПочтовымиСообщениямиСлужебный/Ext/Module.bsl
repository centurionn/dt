...
Процедура ОтправитьСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки) Экспорт
	...
	//Евросиб Коханенко Н.А. 13.09.2024 (( #90611
	МассивАдресовИПредставлений = Новый Массив;
	Если Кому <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивАдресовИПредставлений, Кому, Истина);
	КонецЕсли;
	Если Копии <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивАдресовИПредставлений, Копии, Истина);
	КонецЕсли;
	Если СкрытыеКопии <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивАдресовИПредставлений, СкрытыеКопии, Истина);
	КонецЕсли;
	Если АдресОтвета <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивАдресовИПредставлений, АдресОтвета, Истина);
	КонецЕсли;
    МассивАдресов = Новый Массив;
	Для Каждого Элемент Из МассивАдресовИПредставлений Цикл
		МассивАдресов.Добавить(Элемент.Адрес);
	КонецЦикла; 
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПользователиКонтактнаяИнформация.АдресЭП КАК ЗапрещенныйАдрес
		|ИЗ
		|	Справочник.Пользователи.КонтактнаяИнформация КАК ПользователиКонтактнаяИнформация
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
		|		ПО ПользователиКонтактнаяИнформация.Ссылка = Пользователи.Ссылка
		|			И ПользователиКонтактнаяИнформация.АдресЭП В(&АдресаЭП)
		|			И (Пользователи.Недействителен
		|				ИЛИ Пользователи.ПометкаУдаления)";
	Запрос.УстановитьПараметр("АдресаЭП", МассивАдресов);
	РезультатЗапроса = Запрос.Выполнить();
    СписокЗапрещенныхАдресов = РезультатЗапроса.Выгрузить(); 
	//Евросиб Коханенко Н.А. 13.09.2024 )) #90611 
	
	// Формируем адрес получателя.
	Для Каждого ПочтовыйАдресПолучателя Из Кому Цикл
		//Евросиб Коханенко Н.А. 12.09.2024 (( #90611 
		Если СписокЗапрещенныхАдресов.Найти(ПочтовыйАдресПолучателя.Адрес) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		//Евросиб Коханенко Н.А. 12.09.2024 )) #90611 
	...
	Если ПараметрыОтправки.Свойство("Копии", Копии) Тогда
		Для Каждого ПочтовыйАдресПолучателяКопии Из Копии Цикл
			//Евросиб Коханенко Н.А. 12.09.2024 (( #90611 
			Если СписокЗапрещенныхАдресов.Найти(ПочтовыйАдресПолучателяКопии.Адрес) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			//Евросиб Коханенко Н.А. 12.09.2024 )) #90611 
	...
	Если ПараметрыОтправки.Свойство("СкрытыеКопии", СкрытыеКопии) Тогда
		Для Каждого СведенияОПолучателе Из СкрытыеКопии Цикл
			//Евросиб Коханенко Н.А. 12.09.2024 (( #90611 
			Если СписокЗапрещенныхАдресов.Найти(СведенияОПолучателе.Адрес) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			//Евросиб Коханенко Н.А. 12.09.2024 )) #90611 
	...
	Если ПараметрыОтправки.Свойство("АдресОтвета", АдресОтвета) Тогда
		Для Каждого ПочтовыйАдресОтвета Из АдресОтвета Цикл
			//Евросиб Коханенко Н.А. 12.09.2024 (( #90611 
			Если СписокЗапрещенныхАдресов.Найти(ПочтовыйАдресОтвета.Адрес) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			//Евросиб Коханенко Н.А. 12.09.2024 )) #90611 
...
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли Не РежимТестирования Тогда
			...
			Если ПараметрыЗагрузки.Свойство("АвтоматическиУдалятьСообщенияПриВыбореССервера") Тогда
				АвтоматическиУдалятьСообщенияПриВыбореССервера = ПараметрыЗагрузки.АвтоматическиУдалятьСообщенияПриВыбореССервера;
			КонецЕсли;
