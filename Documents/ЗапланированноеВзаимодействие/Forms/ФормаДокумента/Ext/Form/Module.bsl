...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Объект.Статус = Перечисления.СтатусыЭтапов.Новый;
	КонецЕсли;
	...
	// Заполнение взаимодействия из чата
	Если Параметры.Свойство("ОткрытиеИзЧата") Тогда
		
		Объект.Тема = Параметры.ТекстСообщения;
		Объект.Ответственный = Пользователи.ТекущийПользователь();
		Объект.СоздатьПроизводственнуюЗадачу = Истина;
		Объект.ДатаНачала = ТекущаяДата();
		Объект.ДатаОкончания = ТекущаяДата()+24*3600;
		Объект.Автор = Пользователи.ТекущийПользователь();
		Объект.Дата = ТекущаяДата();
		НСтр = Объект.Участники.Добавить();
		НСтр.Контакт = Пользователи.ТекущийПользователь();
		НСтр.ПредставлениеКонтакта = НСтр.Контакт;
		ГУИДИсточника = Параметры.ГУИДИсточника;
		
	КонецЕсли;
	//Евросиб СубботинВ тикет 0403478 19.06.2025 Начало 
	//Если ЗначениеЗаполнено(Объект.УдалитьПредмет) Тогда
	//	Предмет = Объект.УдалитьПредмет;
	//	Если ТипЗнч(Предмет) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
	//		Элементы.Сделка.Видимость = Истина;
	//		Сделка = Предмет.Сделка;
	//	КонецЕсли;
	//КонецЕсли;
	//Евросиб СубботинВ тикет 0403478  19.06.2025 Конец
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ЗначениеЗаполнено(ГУИДИсточника) и ЗначениеЗаполнено(Объект.Ссылка) Тогда
		СписокУчастников = "";
		Для каждого Стр из Объект.Участники Цикл
			СписокУчастников = СписокУчастников + ?(СписокУчастников = "", "", "; ") + Стр.ПредставлениеКонтакта;
		КонецЦикла;
		
		Оповестить("ЗаписьВзаимодействия", Новый Структура("ВзаимодействиеСсылка, СписокУчастников, ПериодВзаимодействия", Объект.Ссылка, СписокУчастников, ""+Формат(Объект.ДатаНачала, "ДФ=dd.MM.yy")+" - "+Формат(Объект.ДатаОкончания, "ДФ=dd.MM.yy")), ГУИДИсточника);
	КонецЕсли;
