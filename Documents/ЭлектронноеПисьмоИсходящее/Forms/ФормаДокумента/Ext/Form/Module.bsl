...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновлениеИнформационнойБазы.ПроверитьОбъектОбработан(Объект, ЭтотОбъект);
 	...
 	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Параметры.Свойство("ДатаОтправкиРелизаКлиента") Тогда
			ДатаОтправкиРелизаКлиента = Параметры.ДатаОтправкиРелизаКлиента;
		КонецЕсли;
		// Евросиб Фролов)
		ПерваяЗаписьДляРелиза = Истина; //ДИТ Шевченко А.В. 51340 19.10.2022   
		// ДИТ US 72708 ТА 72713 Сержантова А.С. Начало: 11.09.2023 
		Если Параметры.Свойство("ВладельцыПФ") Тогда
			Для каждого Док Из Параметры.ВладельцыПФ Цикл
				НС = Объект.ВладельцыПФ.Добавить();
				НС.Документ = Док; 
			КонецЦикла;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.РассылкаСОграничениями.Видимость = Ложь;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//ДИТ Шевченко А.В. 51340 19.10.2022 -Начало
	Если ПерваяЗаписьДляРелиза Тогда
		Если ТипЗнч(Предмет) = Тип("ДокументСсылка.РелизКлиента") Тогда			
			ДокументОбъект = Предмет.ПолучитьОбъект();
			ДокументОбъект.ПисьмоНаЗапрос = ТекущийОбъект.Ссылка;
			ДокументОбъект.Записать(РежимЗаписиДокумента.Запись);			
		КонецЕсли;	
	КонецЕсли;
	//ДИТ Шевченко А.В. 51340 19.10.2022 -Конец
...
//Евросиб Коханенко Н.А. 22.06.2023 (( Ticket#0314714 
		//НЕ ИнформационнаяБазаФайловая ИЛИ 
//Евросиб Коханенко Н.А. 22.06.2023 )) Ticket#0314714 
		(Объект.ДатаКогдаОтправить <> Дата(1,1,1) И Объект.ДатаКогдаОтправить > ОбщегоНазначенияКлиент.ДатаСеанса())
...
Процедура ОпределитьВидимостьДоступностьЭлементовВЗависимостиОтСтатусаПисьма()
	...
	Если Объект.СтатусПисьма <> Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
		...
		Если Объект.СтатусПисьма = Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Исходящее И ( 
			...
			//НЕ ИнформационнаяБазаФайловая ИЛИ 
			//Евросиб Коханенко Н.А. 22.06.2023 )) Ticket#0314714 
			(Объект.ДатаКогдаОтправить <> Дата(1,1,1) И Объект.ДатаКогдаОтправить > ТекущаяДатаСеанса())
...
Процедура ОбработатьПереданныеПараметры(ПереданныеПараметры)
	...
	Если ПереданныеПараметры.Свойство("Получатель") И ПереданныеПараметры.Получатель <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(ПереданныеПараметры.Получатель) = Тип("СписокЗначений") Тогда
			...
			Если ТипЗнч(ПереданныеПараметры.Предмет) = Тип("ДокументСсылка.РеестрСпециальныхСтавок") Тогда
				ЗаполнитьПолучателейДляСпециальныхСтавок(ПереданныеПараметры);
			Иначе
...
Процедура УстановитьТекстПисьма()
	...
	КонецЕсли;
	//Евросиб - АСуслов 07.08.2020 11:28:24 #
	//(
	Параметры.Свойство("ТекстТаблицаВложений", Объект.ТекстТаблицаВложенныхФайловHTML);
	//)
	//Евросиб - АСуслов 07.08.2020 11:28:24 #
...
//Евросиб Гудкова( 13.10.2020 Ticket№ 0210084
//				Если МетаданныеОбъекта.СтандартныеРеквизиты[Поле.Имя].Тип.Типы()[0] = Тип("Строка") Тогда
				РеквизитПоиска = МетаданныеОбъекта.Реквизиты.Найти(Поле.Имя);
				Если РеквизитПоиска = Неопределено Тогда
					РеквизитПоиска = МетаданныеОбъекта.СтандартныеРеквизиты[Поле.Имя]; 
				КонецЕсли; 
				Если РеквизитПоиска.Тип.Типы()[0] = Тип("Строка") Тогда
// Евросиб Гудкова) 				
...
&НаКлиенте
Процедура ОтправитьСУчетомОграничений(Команда)
	
	Если Параметры.Свойство("МассивКонтактныхЛиц") Тогда
		//  заполнение получателей письма по контактам контрагента
		
		СписокПолучателейКонтрагента = Новый СписокЗначений;
		
		КонтрагентКЛ = Параметры.МассивКонтактныхЛиц;
		...
		Для Каждого Стр Из КонтрагентКЛ Цикл
			...
			СтрокаВложений = Новый Массив;
			
			Для Каждого ЭлемСтруктуры Из Параметры.Вложения Цикл
				
			    ДокДляОтправки = ЭлемСтруктуры.Представление;
				...
				ИначеЕсли НЕ СтрНайти(Врег(ДокДляОтправки), "ПСЦ_ОДНОЯЗЫЧНЫЙ2") = 0 Тогда
				...
				Если ПроверкаВыполненена Тогда
					МассивВложений.Добавить(ЭлемСтруктуры);
					СтрокаВложений.Добавить(ДокДляОтправки);
				КонецЕсли;
			...
			Если МассивВложений.Количество()>0 Тогда
				...
				НовСтр = РассылкаСОграничениями.Добавить();
				НовСтр.Получатель = ТекПолучатель;
				НовСтр.СписокДокументов.ЗагрузитьЗначения(СтрокаВложений);
				НовСтр.Тема = Объект.Тема;
				НовСтр.ТекстПисьма = ТекстПисьма;
			КонецЕсли;
		КонецЦикла;
		
		Элементы.РассылкаСОграничениями.Видимость = Истина;
...
&НаКлиенте
Процедура СохранитьПисьмаСУчетомОграничений(Команда)
	
	Для Каждого Стр ИЗ РассылкаСОграничениями Цикл
		
		Результат = Новый Структура;
		Результат.Вставить("Получатель", Стр.Получатель);
		Результат.Вставить("РассылкаСОграничениями",Истина);
		Результат.Вставить("Тема", Стр.Тема);
		Результат.Вставить("Текст", Стр.ТекстПисьма);
		ВложенияДляПользователя = Новый Массив;
		Для Каждого Элем Из Параметры.Вложения Цикл
			Если НЕ Стр.СписокДокументов.НайтиПоЗначению(Элем.Представление) = Неопределено Тогда
				ВложенияДляПользователя.Добавить(Элем);
			КонецЕсли;
		КонецЦикла;
		Результат.Вставить("Вложения", ВложенияДляПользователя);
...
// Евросиб US 90342 Рыбьяков К.А. 10.03.2026 ((
&НаСервере
Процедура ЗаполнитьПолучателейДляСпециальныхСтавок(ПереданныеПараметры)
	
	АдресИнициатора = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Объект.Автор, Справочники.ВидыКонтактнойИнформации.EmailПользователя);	
	
	Для Каждого ЭлементСписка Из ПереданныеПараметры.Получатель Цикл
		Если ЭлементСписка.Значение = АдресИнициатора Тогда
			Продолжить;
		КонецЕсли;
		НоваяСтрока = Объект.ПолучателиСкрытыхКопий.Добавить();
		НоваяСтрока.Адрес = ЭлементСписка.Значение;
		...
		НоваяСтрока = СписокПолучателей.Добавить();
		НоваяСтрока.ВариантОтправки = НСтр("ru = 'Скрытые'");
		НоваяСтрока.Адрес = ЭлементСписка.Значение;
		НоваяСтрока.Представление = ОбработанноеПредставлениеАдресата(ЭлементСписка.Представление);
	КонецЦикла;
	
	НоваяСтрока = Объект.ПолучателиПисьма.Добавить();
	НоваяСтрока.Адрес = АдресИнициатора;
	
	НоваяСтрока = СписокПолучателей.Добавить();
	НоваяСтрока.ВариантОтправки = НСтр("ru = 'Кому:'");
	НоваяСтрока.Адрес = АдресИнициатора;
	
КонецПроцедуры // Евросиб Рыбьяков К.А. 10.03.2026 ))
